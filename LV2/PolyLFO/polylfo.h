...
#define LFO_URI "http://polyeffects.com/lv2/polylfo"
...
typedef struct _PolyLFO{
	...
	const float *time[MAX_POINTS];
	const float *value[MAX_POINTS];
	const float *style[MAX_POINTS];
	const float *num_points;
	const float *cc_num;
	const float *channel;
	...
	void (*filter_fn) (struct _PolyLFO*, uint32_t, const uint8_t* const, uint32_t);
	void (*preproc_fn)  (struct _PolyLFO*);
	void (*postproc_fn) (struct _PolyLFO*);
	void (*cleanup_fn)  (struct _PolyLFO*);
} PolyLFO;

void forge_midimessage(PolyLFO* self,
		uint32_t tme,
